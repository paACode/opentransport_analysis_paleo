...
Sys.setenv(LANG = "en")
#
train.raw <-  read.csv(file = "raw_transport_2024.csv", sep = ";")

test_that("Shell Scripts to reduce dataset worked?", {
  expect_equal(unique(train.raw$BETREIBER_ABK), "SBB")
  expect_equal(unique(train.raw$HALTESTELLEN_NAME), "Luzern")
  expect_equal(unique(train.raw$PRODUKT_ID), "Zug")
})
