...
In the previous chapter, plots illustrated the number of delays on different days.To enable MadeUpCompany AG to explore the data independently, an interactive R Shiny app was developed. This allows them to uncover patterns that may have gone unnoticed.

The plot below showcases all the data available for exploration . However, due to the high density of information, it is difficult to discern any clear patterns. With the interactive capabilities of the Shiny app, users can now explore the data dynamically and uncover meaningful insights more effectively.
...
```{r Shiny, echo=FALSE, message = FALSE, include=FALSE}
