...
As a first step, a plot was created that provides an overview of SBB's overall performance. It shows that over 35% of trains arrive on time, while slightly more than 60% arrive within a delay of 0 to 5 minutes.
Longer delays account for only 5% of the total. This first view 
...
One might conclude that delays longer than 5 minutes are rare and, therefore, have little impact on production. However, when considering the actual number of delayed trains per year, it becomes clear that the disruption to production could still be significant, as there are over 2,000 trains arriving with extreme delays.

```{r Count of extreme delays, include=TRUE}
subset.extremeDelays <- d.train.compl  %>% 
  filter(delay.calc.min > 5)

ggplot(data = subset.extremeDelays, aes(x = delay.category)) +
  geom_bar(aes(y = ..count..), fill = "skyblue", color = "black") +  # Bar chart
  geom_text(stat = "count", aes(label = ..count..), vjust = -0.25, size = 4, fontface= "bold") +  # Add count labels above bars
  labs(y = "Count", x = "Delay [mins]") +  
  ggtitle("Overview-Distribution of Extreme Delays", subtitle = "Lucerne Train Station, 2024") +
  theme_minimal()
```

Another thing to mention in the Overview Plot is that SBB’s performance might seem poor, with over 60% of trains arriving late. However, a closer look at the 5-minute delay category reveals that more than 50% of these trains are delayed by less than a minute, which puts the initial impression into perspective.
...
ggplot(data =subset.smaller5min, aes(x = delay.calc.min)) +
  ...
  labs(y = "Count[%]", x = "Delay [mins]") +  # Add axis labels
  ggtitle("Closer Look at < 5min category", subtitle = "lucerne train station, 2024") +
  theme_minimal()  # Apply a minimal theme
```
...
Building on the weekday-level analysis, it is important to further investigate specific hours of train arrivals in Lucerne to identify how delay patterns align with key shift changes at MadeUpCompany AG, where shifts start at **06:00, 14:00, and 22:00**, and workers are expected to arrive at least **30 minutes prior**. By focusing on hourly arrival data, we aim to uncover critical time windows where public transport delays may disproportionately impact employee punctuality and production reliability.  
...
The plot illustrates the percentage of delayed trains across different time windows throughout the day, revealing distinct patterns linked to train traffic and operational challenges. For workers starting their **06:00 shift**, the key arrival window is between **05:00 and 05:30**. The plot shows **moderately high delay percentages of around 60% during this time window**, which, although lower than the peak at **06:00-08:00**, still poses a **moderate risk to worker punctuality**. Any delays during this period could lead to last-minute arrivals or missed shifts, suggesting that targeted measures to reduce early morning delays may improve reliability.

The next critical window is for workers starting the **14:00 shift**, who need to arrive between **13:00 and 13:30**. The plot shows **relatively low delay percentages (around 40% to 50%) during this time frame**, indicating **smoother operations** and a **reduced risk of transport-related disruptions** compared to the morning.  

For the **22:00 shift**, workers must arrive between **21:00 and 21:30**. During this window, the plot indicates **delay rates of around 50%**, which are not as severe as during the morning or evening rush hours, but still pose a **potential punctuality risk** due to the cumulative nature of delays from the evening period (17:00 to 20:00).  

These observations highlight the need for **targeted interventions during the early morning (05:00 to 05:30)**, where delays may pose the highest risk to on-time arrivals. Additionally, the **moderate but manageable delays in the evening (21:00 to 21:30)** suggest that ensuring smooth evening operations could prevent issues for the night shift. The **off-peak period before the 14:00 shift** appears less problematic, providing an opportunity to allocate maintenance or optimize train schedules. By focusing on these key time windows, precise countermeasures can be developed to **minimize transport-related disruptions and improve production reliability** for MadeUpCompany AG.
...
```{r Analizing Delays on hourly timeframe basis, echo=FALSE, message = FALSE, include=FALSE}
library(dplyr)
library(lubridate)
library(gt)
