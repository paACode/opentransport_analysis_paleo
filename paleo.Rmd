...
Comments: - Analyze Every month separate with boxplot to investigate if Tue and Fri have throughout the year higher delays - Analyze November in detail since there have been the longest delays. Has it something to do with the Weather?

## Assessing the Impact of Weather on Train Delays

After an initial analysis of the transport data, the focus shifts to a deeper exploration of the weather data.
While the weather data is aggregated by day, the transport data provides delay times for each incoming train. As a first step, the delay times must be aggregated by day, which can be done by calculating the average delay for each day.
...
Plotting the delay as a time series reveals two periods where the average temperature falls below 0°C (highlighted in Blue).

A sharp increase in delays occurs in November, but not in January. The initial drop below freezing in November probably causes failures due to undetected issues in switch heating systems. By January, the railway operator (SBB) is likely better prepared, as the prolonged winter has already exposed and resolved most defects.
...
```
A closer look at November suggests that the initial temperature drop led to the sharp increase in delays. However, daily aggregated delay data lacks detail. For a clearer analysis, weather data should be available at an hourly level to better capture its impact on delays. With such limited data, producing a reliable weather model is not feasible, but a qualitative assessment was possible. SBB also confirms that they have the highest delays in November. For more Information follow this link [here](https://news.sbb.ch/medien/dossiers/dossier/122215/puenktlichkeit#:~:text=Im%20Monaten%20Oktober%20und%20November,wenn%20die%20Gleise%20feucht%20sind).
...
```{r average delays on rainy vs dry days, include=TRUE, echo=TRUE}
...
## => NOT EQUAL, hence difference in delays between "Rainy" and "Dry" days is highly statistically significant
