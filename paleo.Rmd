...
subtitle: "[Public GitHub Repository](https://github.com/paACode/opentransport_analysis_paleo)"
...
The obtained dataset for the year 2024 covers all swisswide available data and therefore is roughly \~150GB. Working with such a big data set in R is very time consuming, impractical and for the scope of this study not necessary. Hence, the dataset was reduce to a manageable size before working with R.
...
require(testthat)   # for unit test
require(dplyr)      # for data wrangling
require(ggplot2)    # for plotting
require(tidyr)      # For reshaping data
require(lubridate)  # for easier handling of date and times
require(patchwork)  # combining different ggplots into a single layout
require(ggcorrplot) # required for correlation matrix
require(shiny)      # allows interactive app
...
Upon reviewing the dataset, several issues were identified that need to be addressed:

-   Trains departing late from Lucerne should be excluded, as only the arrival times are relevant to our analysis.
-   Arrival times that are predicted rather than measured should also be removed, as the focus is exclusively on real data.
...
calc.delay.s <- function(exp.time,
  ...
  test_that("Correct Function inputs?", {
    ...
    expect_equal(
      class(exp.time.format),class(actual.time.format),
      class(exp.time),class(actual.time),"character"
    )
  })
...
d.train.compl <-  d.train.filtered # First copy filtered dataset 
## Add delay in seconds
d.train.compl$delay.calc.s <- calc.delay.s(
  exp.time = d.train.filtered$ANKUNFTSZEIT,
  exp.time.format = "%d.%m.%Y %H:%M",# SBB provides only minute accuracy
...
## Add Weekdays
weekday.order <- c("Monday","Tuesday","Wednesday",
...
In addition to merging and averaging the individual measurement values, the dataset's labels were also renamed for readability. For example, *tre200d0* was renamed to *Lufttemperatur (as shown below)*.

```{r echo=FALSE, include=TRUE, message=FALSE}
## Display weather data for documentation purposes
d.weather %>%
  transmute(
    Datum = Datum,
...
Finally, an average weather dataset for Switzerland for the year 2024 was obtained, which is based on the most important ground measurement stations within the MeteoSwiss network.

## Merge Datasets

To analyze the impact of weather on train delays, it was necessary to merge the **weather dataset** with the **train transport dataset**. Since both datasets used different date formats, a preprocessing step was required to ensure compatibility. By aligning the date formats and performing a **left join**, the weather data was successfully integrated into the transport dataset, allowing for a comprehensive analysis of how weather conditions influence train delays.
...
The merging of the weather and train datasets ensures a comprehensive foundation for analyzing the impact of weather conditions on train delays. With properly aligned data, statistical evaluations and visualizations can now be conducted to identify patterns and key influencing factors. This integrated dataset allows for a more detailed examination of potential disruptions, providing insights into scheduling optimizations and operational adjustments.

# Data Analysis

This chapter examines the temporal and categorical patterns of train delays affecting MadeUpCompany AG employees arriving in Lucerne. By analyzing public transport data from 2024 in combination with Swiss weather data, the study investigates delay frequency, duration, and their distribution across weekdays and months. Key metrics, including total and delayed train counts, delay severity, and weather influence, are evaluated to identify trends that can inform effective countermeasures. Through statistical summaries and visualizations, the analysis aims to provide precise, data-driven recommendations to enhance production reliability by mitigating transport-related disruptions.
...
ggplot(data = d.train.compl, aes(x = delay.category)) +
  ...
  geom_text( aes(y = ..count.. / sum(..count..) * 100, 
        label = ..count..), stat = "count", vjust = -0.5, colour = "blue")  
...
ggplot(data =subset.smaller5min, aes(x = delay.calc.min)) +
  geom_histogram(
    aes(y = ..count.. / sum(..count..) * 100),  # Convert counts to percentages
    binwidth = 1, fill = "skyblue", color = "black", boundary = 0) +  
...
As a further step, the air temperature was plotted againt the delays. It appears that there is no clear, strong linear relationship between air temperature ("Lufttemperatur") and delays. The points are widely scattered, and the regression line appears mostly flat. Additionally, a dense clustering of points around zero on the y-axis indicates that a large number of observations have minimal or no delays, regardless of air temperature. Several noticeable outliers exceed 50-minute delays, but these events seem sporadic and do not follow a clear pattern with air temperature. The air temperature values primarily fall between 0 and 20 degrees, but within this range, no significant change in delays is visually obvious. However, between 0 and -5 degrees, delays appear to be higher, suggesting further analysis, particularly regarding precipitation. Although jittering helps to distinguish overlapping points, the overall impression is that delays are not highly influenced by air temperature.
...
ggplot(d.combined, aes(x = Lufttemperatur, y = delay.calc.min)) +
  ...
  geom_smooth(method = "lm", color = "orange", se = FALSE) + # Fit Linear Model
...
## Temporal Analysis of Train Delays
...
## Only consider delayed trains 
d.delayed_trains <- d.combined %>% 
  filter(delay.category != "on time") # Remove trains that are on time 
## Crate a table with min, max, median, quantiles
...
ggplot(d.delayed_trains, aes(x = season, y = delay.calc.min, fill = season)) +
    ...
    # Outlier plotting does not make sense for jitter plot
    geom_boxplot(outlier.shape = NA) +  
    scale_y_sqrt() 
    # Very Important! Sqrt Scale removes delays < 0 in plot, which would
    # not make much sense in this plot
```

### Monthly Delay Analysis

Building on the seasonal analysis, it is essential to examine train delays on a monthly to uncover persistent delay patterns and identify operational challenges affecting employee punctuality at MadeUpCompany AG. The "Monthly Overview of Train Delays" was chosen to illustrate how delays fluctuate over time, providing insight into critical monthly trends and complementing the broader seasonal findings.

For most months, the average delay time remains relatively stable between 80 and 110 seconds. However, a spike in November is visible. strengthens the hypothesis that higher delays are associated with early-season temperature drops. This pattern suggests potential failures in switch heating systems, leading to freezing-related malfunctions.
...
As a final step in the train delay analysis, the investigation focused on whether certain weekdays exhibit **distinct delay patterns throughout the day**. By analyzing the **percentage of delayed trains for each time window** and comparing daily trends, the goal is to identify differences in delay behavior between weekdays and weekends. This analysis will provide further clarity on how **delay patterns evolve day-to-day** and whether **weekday-specific scheduling adjustments are warranted**.
...
The plot highlights that **Monday through Friday** follow similar delay patterns, with noticeable peaks during the **early morning (03:00-05:00) and evening (17:00-20:00)**. These peaks reflect periods of high train traffic, coinciding with **major shift changes and commuting times for employees**. The delay percentages are generally **higher on Tuesday and Thursday**, confirming earlier observations that these days face **greater operational challenges** and require careful planning of **buffer times and flexible work schedules** to minimize disruptions.

On the other hand, **weekend days—Saturday and Sunday—exhibit relatively stable and lower delay percentages across most time windows**. The absence of major peaks indicates **reduced traffic and fewer operational bottlenecks**. The smoother weekend patterns suggest that optimizing **shift planning on weekends** seems less necessary as its not really impacting employee punctuality.

In conclusion, this final plot demonstrates that **weekday-specific planning is essential**, particularly on **high-risk days like Tuesday and Thursday**. In total, these insights contribute to a **holistic understanding of how delays could affect production** at MadeUpCompany AG and support the development of **effective, data-driven strategies** for mitigating transport-related disruptions.

### Chapter of Choice - R Shiny App
...
### Daily Delays and Weather Impact
...
### Extreme Weather and Train Delays
...
This chapter summarizes the key findings of the analysis.  


Overall, SBB performs well at Lucerne train station, with approximately **95% of trains arriving either on time or with a delay of no more than 5 minutes**. These categories are considered less critical and are unlikely to significantly disrupt production at MadeUpCompany AG. However, the remaining **5% of trains—equivalent to 2,343 annually—experience delays greater than 5 minutes**, posing a potential risk to worker punctuality and production schedules. While 5% may seem small in proportion, the absolute number of delayed trains remains significant, particularly during peak commuting times.

![](Key_Finding_Fall.png)

A seasonal analysis revealed that fall experiences a significantly higher number of long delays (>30 minutes) compared to other seasons. This is surprising, as one might expect winter to have more delays due to snow and ice. However, a closer look at the data identified November as the most delay-prone month, with a sharp increase in long delays. This pattern is likely due to **switch failures and track-related issues caused by wet or freezing conditions**. As temperatures drop for the first time in the season, defects in heating systems (such as defective switch heaters) become apparent, leading to infrastructure-related disruptions. By January, although the cold persists, previous failures have been detected and resolved, leading to fewer delays despite similar weather conditions. This pattern suggests that early-season freezing events require better preparedness, as November consistently experiences the most severe disruptions.



![](Key_Finding_November.png)

A further analysis of delay frequency by time of day identified **optimal commuting windows** where delays are least likely to disrupt shift-based production. **Peak delays occur during midnight (00:00-02:00), early morning (07:00-11:00), and evening (17:00-21:00)**. In contrast, **off-peak periods (03:00-06:00, 12:00-16:00, and 21:00-23:00) show significantly fewer delays**, offering more reliable commuting options.

For MadeUpCompany AG’s shifts at **06:00, 14:00, and 22:00**, the morning commute (05:00-05:30) carries **low risk**, the afternoon window (13:00-13:30) has **slightly higher delays**, and the **highest risk (65% delay rate) occurs during the 21:00-21:30 window**, affecting the night shift. Weekday trends show **Tuesdays and Thursdays experience the most frequent delays**, while weekends exhibit **more stable operations**.

Short delays (5-15 minutes) dominate, but **longer delays (15-30 minutes and >30 minutes) are rare but cluster around peak hours**, posing risks during high-traffic periods. **Aligning shift start times with low-risk commuting windows, monitoring peak risk periods, and advising employees on alternative travel times could help mitigate train delays’ impact on production operations.**


![](Key_Finding_Timewindows.png)


# Conclusion

This analysis provided valuable insights into train delay patterns at Lucerne station and their potential impact on shift-based production at MadeUpCompany AG. The findings show that while 95% of trains arrive within 5 minutes of their scheduled time, the remaining **5%—equivalent to 2,343 delayed trains annually—pose a significant risk**, particularly for employees commuting during peak delay periods.

The highest delays occur in November, likely due to early-season freezing events and switch heating failures. By January, these failures are typically resolved, reducing disruptions despite continued cold weather. Tuesdays and Thursdays experience the most delays, while weekends have smoother transport operations with fewer disruptions. A detailed analysis of train delays by time of day revealed that morning (05:00-05:30) and evening (21:00-21:30) commuting periods are the most delay-prone, suggesting that alternative scheduling strategies or contingency plans could mitigate their impact.

While general weather conditions (such as temperature, precipitation, or snow accumulation) have little correlation with delays, a **notable exception** was identified: when rain occurs at temperatures between -1°C and -5°C, the probability of delays exceeding 5 minutes rises sharply from 3.3% to 36.3%. This is likely due to **freezing rain, icy tracks, or switch failures**, creating localized but severe disruptions. Unlike general rain or cold weather, this specific combination of conditions requires close monitoring, as it has a strong potential to impact worker punctuality.

Based on these findings, several recommendations can be made to improve shift reliability. To minimize disruptions, **November should be considered a high-risk month**, and employees should be encouraged to take earlier trains or alternative transport options where possible. **Low-risk commuting windows were identified between 02:00-06:00, 11:00-16:00, and 20:00-23:00**, which could be leveraged for more reliable commuting schedules. Targeted interventions for peak-risk hours (05:00-05:30 and 21:00-21:30) are recommended, such as real-time monitoring of train delays or adjusting shift start flexibility.

The **lack of high-resolution weather data (e.g., hourly precipitation and temperature records)** limited the ability to create precise weather-delay models. If MadeUpCompany AG aims to predict train delays based on weather, acquiring **more granular weather data** would be necessary.

As part of this project, an interactive R Shiny app was developed, allowing MadeUpCompany AG to explore delay patterns independently. The app enables users to analyze train delays based on specific days, delay categories, and time windows, providing a powerful tool for data-driven decision-making. Future refinements could include real-time data integration or predictive modeling to further optimize scheduling strategies.

In summary, while train delays do impact production, they are **highly dependent on time-of-day and specific weather anomalies rather than general seasonal trends**. By implementing targeted strategies, such as scheduling adjustments during high-risk periods and monitoring specific weather anomalies, MadeUpCompany AG can take proactive steps to minimize disruptions and improve shift reliability.