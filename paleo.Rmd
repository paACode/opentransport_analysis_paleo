...
Vacation peaks are to be expected in :
- Easter Break (April)
- Summer Holidays (July, August)
- Autumn Holidays (October)
- Christmas (Late December and January)
...
plot(only_january$delay.calc.min~only_january$BETRIEBSTAG)
...
```{r average delays on rainy vs dry days}
### Compare average delays on rainy vs dry days !!!! ---------------------------------------------
...
## => INTERESTING but maybe not significant!!!
## OUTPUT: mean_delay in minutes
```
```{r Delays Under Extreme Weather Conditions}
### Delays Under Extreme Weather Conditions !!!!!! ---------------------------------------------


d.combined %>%
  mutate(extreme_weather = ifelse(Niederschlag > 20 | Gesamtschneehöhe > 20, "Extreme", "Normal")) %>%
  ggplot(aes(x = extreme_weather, y = delay.calc.min)) +
  geom_boxplot() +
  labs(title = "Verspätungen bei Extremwetter", x = "Wetterbedingungen", y = "Verspätung (min)")+
    scale_y_sqrt()
