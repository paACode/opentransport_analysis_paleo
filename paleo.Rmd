...
kable(season.tbl, caption = "Summary of Train Delays by Season")
```

However, upon further inspection with a jittered boxplot , we observe that the Fall season exhibits a higher frequency of outliers compared to the other seasons. Interestingly, Winter does not show this pattern, despite the expectation that colder months might lead to more significant delays. This can potentially be explained by the fact that in November, the first snowfall typically occurs, which could challenge operations as SBB (Swiss Federal Railways) adjusts to adverse weather conditions. Further weather analysis in subsequent chapters will explore this aspect in more detail, offering a clearer understanding of how weather impacts delays, especially in the winter months.
...
```{r Delay per Time Window Table, echo=FALSE, message = FALSE, include=FALSE}
library(dplyr)
library(lubridate)
library(gt)
...
As priviously already mentioned is the length of delay not yet taken into account. Therefore it is helpful to investigate in so called outliers or extreme delays. As a first impression we analyzed the dataset by a boxplot.
...
In summary, while most train delays are relatively short, the significant number of extreme delays on certain weekdays, particularly **Thursday and Friday**, indicates the need for **outlier-specific analyses** and corrective measures to address these events effectively.

To further understand the nature of delays and their impact, it is important to investigate how delays vary in severity throughout the day. While previous analyses have focused on general patterns and extreme outliers, examining specific delay categories (5-15 min, 15-30 min, and >30 min) can help identify critical time windows where even moderate or short delays may accumulate and disrupt operations. The following line graph provides insights into the total count of delays per category, highlighting periods of both high and low delay frequencies.

```{r Count of Delayed Trains by Delay Category and Time Window Line Graph, echo=FALSE, message = FALSE, include=TRUE}
library(dplyr)
library(lubridate)
library(ggplot2)
...
ggplot(overview_time_delay_table, aes(x = timeframe, y = delay_count, color = delay.category, group = delay.category)) +
  ...
  geom_point(size = 2) +
  labs(
    title = "Count of Delayed Trains by Delay Category and Time Window",
    x = "Time Window",
...
Comments:
- Analyze Every month separate with boxplot to investigate if Tue and Fri have throughout the year higher delays
- Analyze November in detail since there have been the longest delays. Has it something to do with the Weather?
